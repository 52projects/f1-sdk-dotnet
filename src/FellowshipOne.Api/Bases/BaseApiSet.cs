...
namespace FellowshipOne.Api {
    public abstract class BaseApiSet<T> where T : new() {
        ...
        protected BaseApiSet(OAuthTicket ticket, string baseUrl, ContentType contentType) {
            ...
            _authorizer = new Authorizer(ticket.ConsumerKey, ticket.ConsumerSecret, new AccessToken(ticket.AccessToken, ticket.AccessTokenSecret));
        }
        ...
        public virtual async Task<T> GetAsync(string id) {
            ...
            var item = await _authorizer.ExecuteGetAsync(_baseUrl + string.Format(GetUrl, id));
            return item;
        }
        ...
        protected async Task<S> ExecuteCustomRequestAsync<S>(string url) where S : new() {
            var response = await _authorizer.ExecuteGetAsync(_baseUrl + url);
            return response;
        }
